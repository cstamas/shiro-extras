...
/**
 * The finite space of a java {@code Enum}. Contains all the constants of an enum class.
 * 
...
public class EnumeratedSpace extends AbstractSpace {
  ...
  public boolean isContaining(Spatial spatial) {
    // FIXME: we should also test that the spaces are equivalent, but spatial.getSpace() may return null when assertions are enabled (see AbstractSpatial)
    return spatial instanceof EnumSpatial /* && this.enumeration.equals(((EnumeratedSpace) spatial.getSpace()).getEnumClass()) */;
  }

  @Override
  protected double calculateDistance(Spatial s1, Spatial s2) {
    return Math.abs(((EnumSpatial) s1).getOrdinal() - ((EnumSpatial) s2).getOrdinal());
  }
